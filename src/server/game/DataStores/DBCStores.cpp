...
C3Vector TranslateLocation(C4Vector const* DBCPosition, C3Vector const* basePosition, C3Vector const* splineVector)
{
	C3Vector work;
	...
	float angle = std::atan2(x, y) - DBCPosition->o;
...
void readCamera(M2Camera const* cam, uint32 /*numCams*/, M2Header const* header, CinematicCameraEntry const* dbcentry)
{
	...
	for (uint32 k = 0; k < cam->target_positions.timestamps.number; ++k)
	{
		...
		for (uint32 i = 0; i < targTsArray->number; ++i)
		{
			...
			C3Vector newPos = TranslateLocation(&DBCData, &cam->target_position_base, &targPositions->p0);
	...
	for (uint32 k = 0; k < cam->positions.timestamps.number; ++k)
	{
		...
		{
			// Translate co-ordinates
			C3Vector newPos = TranslateLocation(&DBCData, &cam->position_base, &positions->p0);
			...
			if (targetcam.size() > 0)
			{
				...
				thisCam.locations.o = std::atan2(yDiff, xDiff);

				if (thisCam.locations.o < 0)
					thisCam.locations.o += 2 * float(M_PI);
			}
...
void LoadM2Cameras(const std::string& dataPath)
{
	...
	for (uint32 i = 0; i < sCinematicCameraStore.GetNumRows(); ++i)
	{
		if (CinematicCameraEntry const* dbcentry = sCinematicCameraStore.LookupEntry(i))
		{
			...
			readCamera(cam, header->nCameras, header, dbcentry);
		}
	}
